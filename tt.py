...
if st.button("Prévisualiser le Tableau"):
    if not selected_courses:
        ...
        course_details_data = [[make_para(h) for h in [
            "Nom du Cours", "Objectifs d'apprentissage", "Capacités évaluables", "Connaissances évaluables"]]]
        for course in selected_data:
            course_details_data.append([
                make_para(course["name"]),
                make_para(course.get("objectives", "")),
                make_para(course.get("Capacités évaluables", "")),
                make_para(course.get("Connaissances évaluables", ""))
            ])

        # ---- Third Table: Course Hours & Evaluation Details ----
        headers = ["Nom du Cours","Volume horaire","Pourcentage","Barème (/20)",
        ...
        title = f"<b>Tableau de spécification N°{exam_number} – Semestre {semester}, Année scolaire {academic_year}</b>"
